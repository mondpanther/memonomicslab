...
library(tidyr)
monthlytweets=monthlytweets  %>% 
  mutate(
    across(c(usid,englandid,walesid,scotlandid,ukotherid), ~replace_na(.x, F))
  )
...
cross=monthlytweets%>% select( user_id,  avaxid,  usid,
                  englandid,
                  walesid,scotlandid,ukotherid) %>% group_by(user_id) %>%
...
rr1=lm(avaxid~usid+ukotherid+englandid+walesid+scotlandid)  ,cross)
rr1 %>% summary()


shares=cross %>%  select(lazy_dt())
